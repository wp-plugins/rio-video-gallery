...
<div class="wrap">
 ...
 <form action="" method="post" name="frm_vid_gallery_settings">
  ...
  <table id="tbl_vpostorder_settings">
   <tr>
    <td width="190px">&nbsp;</td>
    <td><input type="radio" <?php if (!empty($vpost_orderby_res) && $vpost_orderby_res == 'asc') { echo 'checked="checked"'; } 	elseif (empty($vpost_orderby_res)) { echo 'checked="checked"'; } ?> value="asc" name="vpost_orderby" >&nbsp;Ascending</td>
    <td><input type="radio" <?php
	if (!empty($vpost_orderby_res) && $vpost_orderby_res == 'desc') { echo 'checked="checked"'; } ?> value="desc" name="vpost_orderby">&nbsp;Descending</td>
 ...
 <?php
	...
	if ($the_query->have_posts()):
		while ($the_query->have_posts()):
         ...
         <article class="video-item" style="width: <?php echo $video_thumb_width_gshort;?>px;">
          <figure> <a href="<?php the_permalink(); ?>">&nbsp;</a>
					<?php if (!empty($provider_shortres) && $provider_shortres == 'youtube') { ?>
   				<img src="http://img.youtube.com/vi/<?php echo $video_id_shortres; ?>/0.jpg" alt="<?php the_title(); ?>" title="<?php the_title(); ?>" width="<?php echo $video_thumb_width_gshort; ?>" height="<?php echo $video_thumb_height_gshort; ?>">
   				<?php }
					...
					?>
   				<img src="<?php echo $thumb; ?>" alt="<?php the_title(); ?>" title="<?php the_title(); ?>" width="<?php 						echo $video_thumb_width_gshort; ?>" height="<?php echo $video_thumb_height_gshort; ?>">
   				<?php } else if (!empty($provider_shortres) && $provider_shortres == 'dailymotion') { ?>
   				<img src="http://www.dailymotion.com/thumbnail/video/<?php echo $video_id_shortres; ?>" alt="<?php the_title(); ?>" title="<?php the_title(); ?>" width="<?php echo $video_thumb_width_gshort; ?>" height="<?php echo $video_thumb_height_gshort; ?>">
         	<?php } ?>
        	</figure>
        	<header>
   				<?php if (!empty($vposted_date_display_gshort)) { ?>
   					<p><span><?php echo human_time_diff(get_the_time('U') , current_time('timestamp')) . ' ago'; ?></span><span><?php
						echo getPostViews($postid); ?> Views</span></p>
   				<?php } ?>
  				</header>
 				</article>
 				...
 				<?php if (!empty($video_layout_gshort) && $video_layout_gshort == 2) { ?>
 					<article class="video-item" style="width: <?php echo $video_thumb_width_gshort; ?>px;">
  					<figure> <a href="<?php the_permalink(); ?>">&nbsp;</a>
   					<?php if (!empty($provider_shortres) && $provider_shortres == 'youtube') { ?>
   					<img src="http://img.youtube.com/vi/<?php echo $video_id_shortres; ?>/0.jpg" alt="<?php the_title(); ?>" title="<?php
						the_title(); ?>" width="<?php echo $video_thumb_width_gshort; ?>" height="<?php echo $video_thumb_height_gshort; ?>">
...
<article>
 ...
 <p><span><?php echo human_time_diff(get_the_time('U', $videoPost_id) , current_time('timestamp')) . ' ago'; ?></span><span><?php
			echo getPostViews($videoPost_id); ?> Views</span></p>
 <?php } ?>
 </header>
...
<?php
// Add js script
function wp_script_file()
{
?>
<script src="<?php echo plugins_url(); ?>/rio-video-gallery/js/video-gallery-script.js"></script>
<?php
}
add_action('wp_head', 'wp_script_file');
/*
|--------------------------------------------------------------------------
| Widget section
|--------------------------------------------------------------------------
	...
	if ($post->post_type == 'video-gallery') {
        ...
        <figure>";
			if (!empty($provider_shortres) && $provider_shortres == 'youtube') {
				$single_template.= "<iframe width='" . $video_sthumb_width_res . "' height='" . $video_sthumb_height_res . "' src='//www.youtube.com/embed/" . $video_id_shortres . "' frameborder='0' allowfullscreen></iframe>";
			}
			else if (!empty($provider_shortres) && $provider_shortres == 'vimeo') {
				$single_template.= "<iframe src='//player.vimeo.com/video/" . $video_id_shortres . "' width='" . $video_sthumb_width_res . "' height='" . $video_sthumb_height_res . "'></iframe>";
			}
			else if (!empty($provider_shortres) && $provider_shortres == 'dailymotion') {
				$single_template.= "<iframe frameborder='0' width='" . $video_sthumb_width_res . "' height='" . $video_sthumb_height_res . "' src='http://www.dailymotion.com/embed/video/" . $video_id_shortres . "'></iframe>";
			}
        ...
        <h4>Related Posts</h4>";
          ...
          <header>
            ...
            <p><span itemprop='datePublished'>Posted ".human_time_diff(get_the_time('U') , current_time('timestamp'))." ago";
						$single_template.= "</span>&nbsp;&nbsp;<span itemprop='playCount'>".getPostViews($post->ID)." Views</span></p>
          </header>
        </article>";
					endwhile;
				else:
